...
import pytest
from textgrad import BlackboxLLM, TextualGradientDescent, Variable, sum
...
def test_import_main_components():
    from textgrad import (BlackboxLLM, EngineLM, TextualGradientDescent,
                          Variable)
    from textgrad.engine.openai import ChatOpenAI
...
def test_formattedllmcall():
    ...
    formatted_llm_call = FormattedLLMCall(
        engine=IdempotentEngine(), format_string=format_string, fields=fields
    )
    inputs = {
...
def test_multimodal():
    ...
    from textgrad.autograd import LLMCall, MultimodalLLMCall
    ...
    with pytest.raises(AssertionError):
        response = MultimodalLLMCall(engine=engine)(
            [
                Variable(4.2, role_description="tst"),
