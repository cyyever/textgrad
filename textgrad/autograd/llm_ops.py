...
class LLMCall(Function):
    def __init__(self, engine: EngineLM, system_prompt: Variable | None = None) -> None:
    ...
    def forward(
        ...
        system_prompt_value = (
            self.system_prompt.value if self.system_prompt is not None else None
        )
    ...
    def backward(
        ...
        if response.get_gradient_text() == "":
            self._backward_through_llm_base(
                children_variables, response, prompt, system_prompt, backward_engine
            )
        else:
    ...
    def _backward_through_llm_chain(
        ...
        for variable in variables:
            ...
            backward_prompt = LLMCall._construct_llm_chain_backward_prompt(
                backward_info
            )
...
class FormattedLLMCall(LLMCall):
    ...
    def forward(
        ...
        response_text = self.engine(
            formatted_input_string, system_prompt=system_prompt_value
        )
        ...
        response = Variable(
            value=response_text,
            predecessors=(
                [self.system_prompt, *input_variables]
                if self.system_prompt
...
class LLMCall_with_in_context_examples(LLMCall):
    def forward(
        ...
        # Stopping criteria could be improved, now is very ugly.
        if (
            "The plan doesn't need to be improved"
            in response_text.split("<FINAL>")[1].split("</FINAL>")[0].strip()
        ):
            response = None
    ...
    def _construct_llm_chain_backward_prompt(backward_info: dict[str, str]) -> str:
        ...
        if len(backward_info["in_context_examples"]) > 0:
            backward_prompt += IN_CONTEXT_EXAMPLE_PROMPT_ADDITION.format(
                **backward_info
            )
        backward_prompt += EVALUATE_VARIABLE_INSTRUCTION.format(**backward_info)
    ...
    def _backward_through_llm_chain(
        ...
        for variable in variables:
            ...
            backward_prompt = (
                LLMCall_with_in_context_examples._construct_llm_chain_backward_prompt(
                    backward_info
                )
            )
    ...
    def _backward_through_llm_base(
        ...
        for variable in variables:
            ...
            backward_prompt = (
                LLMCall_with_in_context_examples._construct_llm_base_backward_prompt(
                    backward_info
                )
            )
