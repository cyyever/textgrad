...
class StringBasedFunction(Function):
    ...
    def backward(
        ...
        if response.get_gradient_text().strip() == "":
            self._backward_through_string_fn_base(
                children_variables, response, inputs, function_purpose, backward_engine
            )
        else:
            self._backward_through_string_fn_chain(
                children_variables, response, inputs, function_purpose, backward_engine
            )

    @staticmethod
    def _construct_string_fn_chain_backward_prompt(
        backward_info: dict[str, str],
    ) -> str:
        conversation = CONVERSATION_TEMPLATE_STRING.format(**backward_info)
        backward_prompt = CONVERSATION_START_INSTRUCTION_STRING_FN_CHAIN.format(
    ...
    def _backward_through_string_fn_chain(
        ...
        inputs_string = "\n\n".join(
            [
                f"**{k.replace('_', ' ').capitalize()}(role: {v.get_role_description()})**: {v.get_short_value()}"
        ...
        for variable in variables:
            ...
            backward_prompt = (
                StringBasedFunction._construct_string_fn_chain_backward_prompt(
                    backward_info
                )
            )
    ...
    def _construct_string_fn_base_backward_prompt(backward_info: dict[str, str]) -> str:
        conversation = CONVERSATION_TEMPLATE_STRING.format(**backward_info)
        backward_prompt = CONVERSATION_START_INSTRUCTION_STRING_FN_BASE.format(
            conversation=conversation, **backward_info
        )
    ...
    def _backward_through_string_fn_base(
        ...
        for variable in variables:
            ...
            backward_prompt = (
                StringBasedFunction._construct_string_fn_base_backward_prompt(
                    backward_info
                )
            )
