    ...
    for g in variable.gradients:
        if variable.gradients_context[g] is None:
            ...
            if isinstance(context["context"], str):
                ...
                context_prompt = GRADIENT_MULTIPART_TEMPLATE.format(
                    **context, feedback=g.value
                )
...
class TextualGradientDescent(Optimizer):
    def __init__(
        ...
        self.in_context_examples = (
            in_context_examples if in_context_examples is not None else []
        )
    ...
    def update_gradient_memory(self, variable: Variable):
        self.gradient_memory_dict[variable].append(
...
class TextualGradientDescentwithMomentum(Optimizer):
    def __init__(
        ...
        logger.info(
            f"TextualGradientDescent initialized with momentum window: {momentum_window}"
        )
    ...
    def step(self):
        for idx, parameter in enumerate(self.parameters):
            ...
            prompt_update_parameter = self._update_prompt(
